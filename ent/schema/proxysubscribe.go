...
func (ProxySubscribe) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Text("short_description").Optional().Nillable().Comment("订阅套餐卡片简介"),
		field.Text("features").Optional().Nillable().Comment("订阅套餐亮点配置"),
		field.String("detail_format").MaxLen(32).Default("markdown").Comment("订阅套餐详情格式"),
		field.Text("detail_content").Optional().Nillable().Comment("订阅套餐详情正文"),
